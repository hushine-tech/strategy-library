...
def test_parse_declared_inputs_normalizes_exchange_and_market():
    inputs = parse_declared_inputs([
        {"exchange": "Binance", "market": "futures", "symbol": "btcusdt", "interval": "1m"},
    ])
    assert inputs == [
...
def test_input_view_returns_latest_tick_by_market_symbol_interval():
    ...
    assert view.market["perpetual_futures"].symbol["BTCUSDT"].interval["1m"] == tick
...
def test_input_view_ignores_undeclared_ticks():
    ...
    assert view.market["perpetual_futures"].symbol["ETHUSDT"].interval["1m"] is None


def test_input_view_normalizes_public_strategy_input_keys():
    view = InputView([StrategyInput(exchange="Binance", market="Futures", symbol="btcusdt", interval="1m")])
