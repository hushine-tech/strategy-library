"""Pure-Python Binance Spot filter evaluator used by offline replay.

The evaluator consumes package-owned immutable facts generated by core-service. It performs
...
def evaluate(
    ...
    """Evaluate one replay order without importing any runtime or network client."""
    facts = dict(immutable_facts or {})
    facts["open_orders"] = list(open_orders)
    assets = getattr(wallet, "assets", {}) or {}
    balances = [
        {
            "asset": str(asset).strip().upper(),
            "available_decimal": str(getattr(balance, "free", "0")),
            "locked_decimal": str(getattr(balance, "locked", "0")),
        }
        for asset, balance in assets.items()
    ]
    return evaluate_spot_filter_vector({
        "request": {
            ...
            "qty_decimal": str(getattr(order, "qty", "") or "").strip(),
            "price_decimal": getattr(order, "price", None),
